...
const caesarModule = (function () {
  ...
  function caesar(input, shift = 0, encode = true) {
    //if an invalid shift length is entered, it will return false. 
    if (shift === 0 || shift < -25 || shift > 25) return false;
    //if the shift argument is negative (characters will be shifted to the left) and encode arg is true, run the following...
    if (shift < 0 && encode){
      shift = 26 + (shift % 26);
    }
    ...
    for(let char of input){
      ...
      //takes the result string and uses the String.fromCharCode method, passed in our character code, to extract a string value that will have been shifted from the input, and adds this value to the result string.
      result += String.fromCharCode(characterCode);
      //if encode is set to false, this code will run instead. 
    } else {
      if(characterCode >= 97 && characterCode <= 122){
        characterCode = (characterCode -97 - shift) % 26 + 97;
        if(characterCode < 97){
